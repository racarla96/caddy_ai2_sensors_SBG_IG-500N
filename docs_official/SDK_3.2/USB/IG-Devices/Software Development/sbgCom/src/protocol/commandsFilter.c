 ...
 *	\return								SBG_NO_ERROR if no error.
 */
SbgErrorCode sbgSetHeaveConf(SbgProtocolHandle handle, bool enableHeave, float heavePeriod)
{
	SbgErrorCode error;
	uint8 heaveConf[8];
	...
	if (handle != SBG_INVALID_PROTOCOL_HANDLE)
	{
		...
		*((uint32*)(heaveConf+sizeof(uint8))) = sbgHostToTargetFloat(handle->targetOutputMode, heavePeriod);
 ...
 *	\return								SBG_NO_ERROR if no error.
 */
SbgErrorCode sbgGetHeaveConf(SbgProtocolHandle handle, bool *pEnableHeave, float *pHeavePeriod)
{
	uint8 cmd;
	uint16 size;
	uint8 heaveConf[8];
		...
		if (error == SBG_NO_ERROR)
		{
			...
			if (error == SBG_NO_ERROR)
			{
				if ( (cmd == SBG_RET_HEAVE_CONF) && (size == sizeof(heaveConf)) )
				{
					...
					if (pHeavePeriod)
					{
						*pHeavePeriod = sbgTargetToHostFloat(handle->targetOutputMode, *((uint32*)(heaveConf+sizeof(uint8))));
					}
				}
				else
				{
