...
#else
	#define SBG_DELETE(p)		if (p){free((p)); (p) = NULL;}
	#define SBG_DELETE_ARRAY(p)	if (p){free((p)); (p) = NULL;}
	#define SBG_FREE(p)			if (p){free((p)); (p) = NULL;}
	#define SBG_FREE_ARRAY(p)	if (p){free((p)); (p) = NULL;}
#endif
...
#include <sys/types.h>
#ifndef __cplusplus
typedef	u_int8_t	bool;				//  8 bits
#endif
typedef u_int8_t	uint8;				//  8 bits
typedef u_int16_t	uint16;				// 16 bits
typedef u_int32_t	uint32;				// 32 bits
typedef u_int64_t	uint64;				// 64 bits
...
typedef enum _SbgErrorCode
{
	...
	SBG_INVALID_CRC,						/*!< Our received frame has an invalid CRC. */
	SBG_INVALID_FRAME,						/*!< Our received frame is invalid <br> */
	...
	SBG_OPERATION_CANCELLED,				/*!< An operation was cancelled. */
